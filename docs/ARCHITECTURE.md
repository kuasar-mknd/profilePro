...
### 2. Database & Persistence Layer (No Prisma/ORM)

Because this is a completely static frontend project generated by Astro, it does not connect to a Prisma database or utilize any ORM (Object-Relational Mapping). All project data and content is sourced statically from Markdown (`.md`/`.mdx`) files located in the `src/content/` directory.
