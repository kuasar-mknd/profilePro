...
*(Note: No Prisma `DATABASE_URL` or Hono backend environment variables are required as this is a fully static frontend application built on Astro. There is no Hono framework used in this repository. The Cloudflare analytics token is strictly optional).*
