...
const CONFIG = {
  ...
  // ⚡ Bolt: Concurrency limit for parallel processing
  concurrency: 4,
};
...
async function processImages() {
  ...
  console.log(`⚡ Mode: Parallèle (x${CONFIG.concurrency})`);
  ...
  // ⚡ Bolt: Parallel Processing using Worker Pattern
  // This ensures we process N images at a time without overloading memory
  const queue = [...files];
  const workers = Array(Math.min(files.length, CONFIG.concurrency))
    .fill(null)
    .map(async () => {
