...
## 2024-05-25 - Enhanced Input Sanitization
...
## 2025-02-18 - NaN Comparison Vulnerability

**Vulnerability:** Input validation logic relying on `parseInt` without checking for `NaN` allows bypassing security thresholds. Comparison operations like `NaN < limit` evaluate to `false`, causing "fail-open" behavior in spam checks.
**Learning:** JavaScript's `NaN` is not ordered relative to numbers. Security checks must explicitly handle invalid numeric input rather than relying on implicit comparison failures.
**Prevention:** Always use `isNaN()` to validate parsed numbers before using them in security-critical logic.
