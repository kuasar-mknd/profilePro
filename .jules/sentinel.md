...
## 2025-02-19 - Logic Bypass in Contact Form

**Vulnerability:** Numeric input validation (timestamp and rate limits) relied on `parseInt` without checking for `NaN`. Since `NaN < number` is always false, an attacker could bypass spam and rate limit checks by providing non-numeric inputs.
**Learning:** Always use `isNaN()` to validate the result of `parseInt()` or `Number()` before using it in comparison logic, especially for security-critical checks.
**Prevention:** Added explicit `isNaN()` checks to `src/components/common/ContactForm.astro`.
