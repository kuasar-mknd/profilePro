...
## 2025-02-19 - Protocol-relative URL Validation Gap

**Vulnerability:** `isValidUrl` and `sanitizeUrl` were intended to block protocol-relative URLs (`//`) but the regex validation (`^/`) unintentionally matched them.
**Learning:** Standard regex anchors for relative paths must be paired with explicit negative checks for `//` to prevent bypassing protocol restrictions or open redirects.
**Prevention:** Explicitly check `startsWith('//')` before allowing paths starting with `/`.
