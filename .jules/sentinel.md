...
## 2025-02-18 - Path Traversal Prevention

**Vulnerability:** Missing sanitization for filenames can lead to directory traversal attacks (e.g., `../../etc/passwd`) if file system operations are introduced.
**Learning:** `sanitizeFilename` must not only remove `..` but also enforce a strict allowlist and strip separators *before* usage to prevent obfuscation bypasses.
**Prevention:** Implemented strict `sanitizeFilename` utility in `src/utils/security.ts`.
