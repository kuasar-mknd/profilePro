...
## 2025-02-18 - Client-Side DOM Injection

**Vulnerability:** Usage of `innerHTML` for UI updates prevents strict CSP (Trusted Types) and risks XSS if inputs become dynamic.
**Learning:** Even for static content, `innerHTML` creates technical debt for security audits.
**Prevention:** Use `document.createElementNS` (for SVGs) and `replaceChildren` to safely update DOM nodes without invoking the HTML parser.
