...
## 2026-01-20 - Protocol-Relative URL Open Redirects

**Vulnerability:** `isValidUrl` and `sanitizeUrl` utilities allowed protocol-relative URLs (`//example.com`) which are technically valid but can be exploited for Open Redirects or Phishing when users expect internal paths.
**Learning:** Standard regex for URLs often allows `//` as a valid protocol-relative path. Explicit checks are needed to enforce strictly safe protocols.
**Prevention:** Added an explicit check `url.startsWith("//")` to reject these URLs in security utilities.
