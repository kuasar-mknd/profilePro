...
## 2025-02-18 - Component Prop Type Safety

**Vulnerability:** Loose typing in UI components (e.g. `any` or missing Props) can lead to unexpected behavior or type confusion vulnerabilities if data structures change.
**Learning:** Enforcing strict `interface Props` in Astro components acts as a contract that validates data at the boundary, preventing invalid data from rendering and crashing the build or runtime.
**Prevention:** Use explicit `interface Props` for all Astro components, especially those accepting external data (collections).
