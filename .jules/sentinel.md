...
**Vulnerability:** Protocol-relative URLs (starting with `//`) can be used to bypass relative path checks (like `startsWith('/')`) and lead to Open Redirects or loading external resources when internal ones are expected.
**Learning:** Standard regexes often miss this edge case. `//example.com` is a valid URL that inherits the current protocol (http/https) but points to an external domain.
**Prevention:** Explicitly check for and block strings starting with `//` in URL sanitization/validation logic before allowing relative paths.
