...
## 2026-02-04 - Localized Date Accessibility
**Learning:** Visual design often favors compact numeric dates (e.g., `dd-mm-yyyy`) which are ambiguous for screen readers. `Intl.DateTimeFormat` allows generating robust, localized verbose strings for `aria-label` without importing heavy libraries.
**Action:** Always wrap numeric `<time>` elements with a verbose `aria-label` using `Intl` for the site's locale.
