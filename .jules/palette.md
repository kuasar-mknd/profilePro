...
## 2025-02-18 - Keyboard Navigation Hints
**Learning:** Adding visual hints (kbd) for keyboard shortcuts (arrows, J/K) improves discoverability for power users without cluttering the UI for mobile users (hidden on mobile).
**Action:** When implementing shortcuts, always pair them with a visual hint in the UI, preferably using the `<kbd>` element styled to match the design system.
