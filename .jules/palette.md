...
## 2025-05-23 - Keyboard Shortcut Discoverability
**Learning:** To improve discoverability of keyboard shortcuts for both sighted and screen reader users, supplement the `aria-keyshortcuts` attribute with a visual hint in the element's `title` attribute (e.g., "Close (Esc)").
**Action:** When implementing interactive controls with keyboard shortcuts, ensure the tooltip explicitly mentions the key.
