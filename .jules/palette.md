...
## 2025-02-22 - Interactive Metadata & Dark Mode Contrast
**Learning:** Metadata tags (like project categories) are often perceived as clickable navigation elements. Making them links enhances discovery. Also, default component variants must explicitly handle dark mode to avoid low contrast issues (e.g. indigo text on black background).
**Action:** Ensure `Tag` components accept `href` and verify default color variants against dark backgrounds using `dark:` modifiers.
